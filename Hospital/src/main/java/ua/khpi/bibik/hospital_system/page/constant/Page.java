package ua.khpi.bibik.hospital_system.page.constant;

public final class Page {

	private Page() {
		// empty
	}
	
	public static final String ADMIN = "path.page.admin";
	public static final String DOCTOR = "path.page.doctor";
	public static final String HOME = "path.page.home";
	public static final String LOGIN = "path.page.login";
	public static final String NEW_DOCTOR = "path.page.new.user.doctor";
	public static final String NEW_PATIENT = "path.page.new.user.patient";
	public static final String PATIENT = "path.page.patient";
	public static final String PATIENT_USER_LIST = "path.page.user.patients";
	public static final String EDIT_DOCTOR = "path.page.user.edit.doctor";
	public static final String EDIT_PATIENT = "path.page.user.edit.patient";
	public static final String PATIENT_INFO = "path.page.user.patient.info";
	public static final String NEW_APPOINTMENT = "path.page.appointment.new";

}
